...
2024.01.18 
- 优化代码逻辑与通知样式,增加时光间、补签卡、捡瓶子任务
- 若账号未打开备份，仅能完成部分任务
- 脚本仍在测试阶段，若有任何问题，请在@NobyDa_Chat及时反馈
...
class UserInfo {
    ...
    async finishDeviceRoomTask() {
        try {
            ...
            //debug(deviceList);
            $.log(`⏰ 开始执行时光设备间备份任务\n`);
            let rewardCountToday = await this.getDeviceRoomInfo();
    ...
    async getDeviceRoomInfo() {
        try {
            ...
            //post方法
            let { result, message } = await this.Request(options);
            DoubleLog(`时光间: 总共领取${result?.rewardTotalSize}MB,今日领取次数：${result?.rewardCountToday}`);
            return result?.rewardCountToday;
